...
#pragma omp parallel
  ...
  const GraphElem nvLocal = g.getNumVertices();
  const GraphElem neLocal = g.getNumEdges();
  const GraphElem numGhosts = dg->getNumGhosts(me);
  // std::cout << "nvLocal[" << nvLocal<< "]; " 
  //              "neLocal[" << neLocal<< "]; "
  //              "numGhosts[" << numGhosts << "];" << std::endl;
  CuVite::GpuGraph gpu_graph(nvLocal, neLocal, numGhosts);
  gpu_graph.devMemAllocate();
  gpu_graph.pinnedAndDevMemAllocate();
  // gpu_graph.freeDev_ModCurrComm(); // this works
  GraphElem vector_size = gpu_graph.getRefVecSize();
...
#ifdef RUNONGPU
            currMod = distLouvainMethod(me, nprocs, *dg, ssz, rsz, ssizes, rsizes,
                    svdata, rvdata, cvect, currMod, threshold, iters, gpu_graph);
